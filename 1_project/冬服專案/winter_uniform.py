import pandas as pd
import openpyxl 
""" 將資料檔案(F表、P表)置放於 D:/冬服/目錄下 """

##1 將調查之需求數量/型號變更、統計後鼓勵金 置入產出之excel需求總表活頁簿
df_P = []
df_P = pd.read_excel('D:/冬服/1原始名單_P.xlsx',sheet_name=0, header=0,dtype={10:str}) 
""" header=0代表Row(0-indexed)--dtype須設第10欄/型號變更欄為字串 否則會報錯 """
df_F = pd.read_excel('D:/冬服/2需求調查_F.xlsx',header=0) #F代表 Feed(餵資料者)

df_P["鼓 勵 金"] = '' #新增 鼓 勵 金欄
df_P["款別有誤"] = '' #新增 款別有誤欄
...
writer = pd.ExcelWriter(filename2, engine='openpyxl') #以Pandas創O2.xlsx空表
writer.book = wb #O2.xlsx(write)置入O1.xlsx活頁簿
...
for groupname, groupdf in df.groupby('款別'): 
	...
	groupdf.to_excel(writer,sheet_name=groupname,index=False) #依款別複製活頁簿入writer
	for i in range(4): #輸入本次發放品項數(4)
		groupdf1 = groupdf.iloc[:, [0,1,2,3,4,5,i+6,10,11]]
		item = groupdf.columns.values[i+6] #取得欄標題名(如長襯)
		groupdf1.to_excel(writer,sheet_name = groupname+'-'+str(item),index=False) #依品項複製活頁簿入writer
writer.save()
wb.close()